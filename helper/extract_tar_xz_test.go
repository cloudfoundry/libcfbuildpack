...
func TestExtractTarXz(t *testing.T) {
	spec.Run(t, "ExtractTarXz", func(t *testing.T, _ spec.G, it spec.S) {
		...
		it("extracts the archive", func() {
			g.Expect(helper.ExtractTarXz(filepath.Join("testdata", "test-archive.tar.xz"), root, 0)).To(gomega.Succeed())
		...
		it("skips stripped components", func() {
			g.Expect(helper.ExtractTarXz(filepath.Join("testdata", "test-archive.tar.xz"), root, 1)).To(gomega.Succeed())
